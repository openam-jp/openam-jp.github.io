...
調査の結果、当コンソーシアムの管理対象プロダクトにおける未公開の脆弱性と確認された場合、CNA として該当の脆弱性に対し CVE ID を割り当て、報告者へ通知します。

### 5. セキュリティアドバイザリおよび CVE Record の公開

対策の準備が整い次第、当サイトの [セキュリティアドバイザリー一覧](../Advisories) にて CVE ID を含めたアドバイザリを公開します。同時に、CVE Services へ CVE Record を登録・公開します。

なお、セキュリティアドバイザリーおよび CVE Record の公開時期については、報告者と事前に共有・調整を行います。
